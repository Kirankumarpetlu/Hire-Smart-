...
from transformers import AutoTokenizer, AutoModelForCausalLM, pipeline, BitsAndBytesConfig # <<<<< IMPORT UPDATED
from peft import PeftModel
from crewai import Agent, Task, Crew

# -----------------------------
# HUGGING FACE MODEL CONFIG (<<<<< SECTION UPDATED)
# -----------------------------
...
base_model = AutoModelForCausalLM.from_pretrained(
    BASE_MODEL_ID,
    token=HF_TOKEN, # <-- REMOVED! Use 'huggingface-cli login' instead.
    trust_remote_code=True,
    quantization_config=bnb_config,  # <-- ADDED for 4-bit loading
    device_map="auto"                # <-- ADDED to manage device memory
)
...
pipe = pipeline(
    ...
    tokenizer=tokenizer, 
    trust_remote_code=True,
    device_map="auto" # <-- ADDED for consistency
)
...
#         for task in [parser_task, skill_task, proficiency_task, matcher_task, ranker_task]:
#             task.output = None 

#         # Run the crew
#         result_output = crew.run([parser_task, skill_task, proficiency_task, matcher_task, ranker_task])
...
#         # The final task's output is in `result_output`
...
# -----------------------------
# TASK PIPELINE (<<<<< UPDATED FUNCTION)
# -----------------------------
...
def analyze_resume_with_crew(jd_text, resume_text, resume_name):
    ...
    try:
        ...
        # This replaces the old global crew object for this function
...
# -----------------------------
# ASYNC MULTI-RESUME HANDLER (<<<<< UPDATED FUNCTION)
# -----------------------------
async def rank_resumes(jd_text, upload_files):
    ...
    for f in upload_files:
        ...
        # This sends the filename to analyze_resume_with_crew
...
async def rank_resumes(jd_text, upload_files):
    ...
    for f in upload_files:
        resume_text = parse_resume_file(f)
        if not resume_text: # Skip if parsing failed
            # Try to get a filename for the error message
            name = getattr(f, "name", "unknown file")
            print(f"Skipping file {name} due to parsing error or empty content.")
            continue
